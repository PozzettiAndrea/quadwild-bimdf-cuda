...
void RXMeshDynamic::update_launch_box(
    ...
    std::function<size_t(uint32_t, uint32_t, uint32_t)> user_shmem) const
{
    // Query device max shared memory optin instead of hardcoding
    int dev_id = 0;
    cudaGetDevice(&dev_id);
    int max_optin = 0;
    cudaDeviceGetAttribute(&max_optin,
        cudaDevAttrMaxSharedMemoryPerBlockOptin, dev_id);
    // Use min of device limit and desired 89KB
    int max_shmem_bytes = std::min(max_optin, 89 * 1024);
    cudaError_t attr_err = cudaFuncSetAttribute(
        kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shmem_bytes);
    if (attr_err != cudaSuccess) {
        // Function pointer may not be valid (separable compilation issue)
        // Try with default 48KB limit
        max_shmem_bytes = 48 * 1024;
        attr_err = cudaFuncSetAttribute(
            kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shmem_bytes);
        if (attr_err != cudaSuccess) {
            RXMESH_WARN("cudaFuncSetAttribute failed: {} — using default shmem",
